...
<!-- Content Header (Page header) -->
    <section class="content-header">
        ...
        <li class="active"><a href="{{url('statement-mbti')}}">Statement</a></li>
        </ol>
    </section>
        ...
        <div class="box">
            <div class="box-body">
                ...
                <div class="table-responsive">
                    <table id="tb_statement_mbti" class="table table-bordered table-striped" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                            ...
                            @foreach ($statement as $item)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <table>
                                            <tr>
                                                <td><span>{{$item->getNamaKategoriMBTI_A->nama_kategori}}</span></td>
                                                <!--<td><span>{{$item->id_kategoriA}}</span></td>-->
                                            </tr>
                                            <tr>
                                                <td><span>{{$item->getNamaKategoriMBTI_B->nama_kategori}}</span></td>
                                                <!--<td><span>{{$item->id_kategoriB}}</span></td>-->
                                            </tr>
                                          </table>
                                        </center>
                                    </td>
    ...
    @include('hrd.psikotes.MBTI.masterdata.statement.create')
    @include('hrd.psikotes.MBTI.masterdata.statement.edit')
...
<script>
    $('.select2').select2();
    $(function(){
        var mainTable = $('#tb_statement_mbti').DataTable();
        var selectedRow;
        $.ajaxSetup({
            headers: {
        ...
        $('#tb_statement_mbti').on('click', '.delete', function (e) {
            e.preventDefault();
        ...
        // edit data kategori 
        $('#tb_statement_mbti').on('click', '#editStatement_', function(e){
            e.preventDefault();

            var id = $(this).data('id');
            console.log("id>>>", id);

            $.get('statement-mbti/'+id , function(data){
