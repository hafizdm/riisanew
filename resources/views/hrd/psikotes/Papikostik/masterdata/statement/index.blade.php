...
<!-- Content Header (Page header) -->
    <section class="content-header">
        ...
        <li class="active"><a href="{{url('statement-papikostik')}}">Statement</a></li>
        </ol>
    </section>
        ...
        <div class="box">
            <div class="box-body">
                ...
                <div class="table-responsive">
                    <table id="tb_statement_papikostik" class="table table-bordered table-striped" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                            ...
                            @foreach ($statement as $item)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <table>
                                            <tr>
                                                {{-- <td><center>{{$item->getNamaKategoriA->nama_kategori}}</center></td> --}}
                                                <td><span>A</span></td>
                                                {{-- <td> - {{$item->id_kategoriA}}</td> --}}
                                            </tr>
                                            <tr>
                                                {{-- <td><center>{{$item->getNamaKategoriB->nama_kategori}}</center></td> --}}
                                                <td><span>B</span></td>
                                                {{-- <td> - {{$item->id_kategoriB}}</td> --}}
                                            </tr>
                                          </table>
                                        </center>
                                    </td>
                                    <td>
                                        <table>
                                            <tr>
                                                <td style="text-align: justify">{{$item->pernyataanA}}</td>
                                            </tr>
                                            <tr>
                                                <td style="text-align: justify">{{$item->pernyataanB}}</td>
                                            </tr>
                                        </table>
                                    </td>
    ...
    @include('hrd.psikotes.Papikostik.masterdata.statement.create')
    @include('hrd.psikotes.Papikostik.masterdata.statement.edit')
...
<script>
    $('.select2').select2();
    $(function(){
        var mainTable = $('#tb_statement_papikostik').DataTable();
        var selectedRow;
        $.ajaxSetup({
            headers: {
        ...
        $('#tb_statement_papikostik').on('click', '.delete', function (e) {
            e.preventDefault();
        ...
        // edit data kategori 
        $('#tb_statement_papikostik').on('click', '#editStatement_', function(e){
            e.preventDefault();

            var id = $(this).data('id');
            console.log("id>>>", id);

            $.get('statement-papikostik/'+id , function(data){
                $('editStatementBtn').val("edit-statement");
                $('statement_modal').modal('show');
                $('#id').val(data.data.id);
                $('#edit_id_kategoriA').val(data.data.id_kategoriA);
                $('#edit_pernyataanA').val(data.data.pernyataanA);
                $('#edit_id_kategoriB').val(data.data.id_kategoriB);
                $('#edit_pernyataanB').val(data.data.pernyataanB);
        ...
        $('#editStatementBtn').click(function(e){
                ...
                var edit_pernyataanA =  $("#edit_pernyataanA").val();
                var edit_id_kategoriB =  $("#edit_id_kategoriB").val();
                var edit_pernyataanB =  $("#edit_pernyataanB").val();
                var edit_id_soal =  $("#edit_id_soal").val();

                // console.log(edit_pernyataan)
                $.ajax({
                    url:"statement-papikostik/update/"+id,
                    type: "POST",
                    data: {
                        id : id,
                        edit_id_kategoriA : edit_id_kategoriA,
                        edit_pernyataanA : edit_pernyataanA,
                        edit_id_kategoriB : edit_id_kategoriB,
                        edit_pernyataanB : edit_pernyataanB,
