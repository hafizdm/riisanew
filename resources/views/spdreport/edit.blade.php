...
@php
 $isEnabled = $spd->employee->spd_report_to != Auth::user()->user_login->id && $spd->spdReport->spdReportApproval->status == 0;   
@endphp
@section('content')
    ...
    <div class="col-xs-12 col-xl-8 col-md-8">
        ...
        <div class="box-body">
            ...
            <div class="col-xs-12 col-xl-6 col-lg-6">
                <form role="form" name="formdata" method="post" enctype="multipart/form-data" action="{{route('update-report', $spd->id)}}">
                    @method('PATCH')
                    @csrf
            ...
            <div class="col-xs-12 col-xl-6 col-lg-6">
                ...
                {{-- @dump($spd->spdReport->report_tgl_keberangkatan) --}}
                <div class="form-group report_tgl_keberangkatan_wrapper">
                    <label for="report_tgl_keberangkatan">Date Departure Report*</label>
                    <input type="date" data-date-format="yyyy/mm/dd" class="form-control" id="report_tgl_keberangkatan" {{ $isEnabled ? '' : 'readonly' }} name="report_tgl_keberangkatan" value="{{$spd->spdReport->report_tgl_keberangkatan->format('Y-m-d')}}" placeholder="yyyy/mm/dd" required >
                    
                </div>

                <div class="form-group report_tgl_pulang_wrapper">
                    <label for="report_tgl_pulang">Date Return Report*</label>
                    <input type="date" data-date-format="yyyy/mm/dd" class="form-control" id="report_tgl_pulang" {{ $isEnabled ? '' : 'readonly' }} name="report_tgl_pulang" value="{{$spd->spdReport->report_tgl_pulang->format('Y-m-d')}}" placeholder="yyyy/mm/dd" required >
                ...
                <div class="form-group cash_out_wrapper">
                    ...
                    <input type="number" id="cash_out" name="cash_out"  value="{{$spd->spdReport->cash_out}}" {{ $isEnabled ? 'required' : 'readonly' }} class="form-control"  />                     
                </div>
                ...
                <div class="form-group upload_report_wrapper">
                    ...
                    <input type="file" id="upload_report" name="upload_report"  {{ $isEnabled ? '' : 'readonly' }} class="form-control" />                      
                </div>
                ...
                <a href="{{url("add-report")}}" class="btn btn-danger">Batal</a>
                @elseif($spd->spdReport && $spd->spdReport->status == 0)
                <div class="dropdown">
...
$('.approved_spd_report').click(function(){
    $.ajaxSetup({
        ...
        $.ajax({
            url: "/spd-report/{{$spd->id}}/approve",
            type: 'GET',
            success: function(data) {
                window.location.href = '/spd-report';
            }
        });
    });
